    ...
    if ($method === 'POST') {
        switch ($currentScript) {
            ...
            case 'register_user.php':
                $newUser = htmlspecialchars($req['login'] ?? '');
                $newRole = htmlspecialchars($req['role'] ?? 'manager');
                logAction('INSERT', 'users', "Администратор зарегистрировал нового пользователя: '{$newUser}' с ролью [{$newRole}]");
                break;

            case 'update_user.php':
                $targetId = (int)($req['id'] ?? 0);
                $targetLogin = htmlspecialchars($req['login'] ?? '');
                $targetRole = htmlspecialchars($req['role'] ?? '');
                logAction('UPDATE', 'users', "Изменены данные пользователя '{$targetLogin}' (ID: {$targetId}, Новая роль: [{$targetRole}])");
                break;

            case 'force-fix.php':
                logAction('UPDATE', 'system', "Запущен скрипт принудительного исправления структуры БД (force-fix.php)");
                break;

            case 'rates.php':
                logAction('UPDATE', 'system', "Менеджер инициировал ручное обновление курсов валют");
                break;

            case 'upload_ttn_pdf.php':
                $ttnId = (int)($req['ttn_id'] ?? 0);
                $fileName = htmlspecialchars($_FILES[key($_FILES)]['name'] ?? 'документ');
            ...
            case 'update_cell.php':
                ...
                if ($field === 'is_contract_signed' && (int)$val === 0) {
                    ...
                    logAction('UPDATE', 'clients', "Отредактирован client ID: {$id}. Изменено поле [{$field}] на '{$val}'");
                }
                break;
            ...
            case 'add_row.php':
                logAction('INSERT', 'clients', "Инициировано создание нового лида (пустой строки)");
                break;
            ...
            case 'save_ttn.php':
                ...
                if ($ttnId > 0) {
                    logAction('UPDATE', 'project_ttns', "Изменена ТТН №{$num} (ID ТТН: {$ttnId}): сумма {$amt} {$cur}");
                } else {
                    logAction('INSERT', 'project_ttns', "Добавлена ТТН №{$num} по договору ID {$projId}: сумма {$amt} {$cur}");
                }
                break;
            ...
            case 'upload_scan.php':
                ...
                if (isset($req['action_mode']) && $req['action_mode'] === 'delete_contract_scan_full') {
                    logAction('DELETE', 'projects', "Полностью удалена скан-копия договора ID {$pid}");
                } else {
                    $fileName = htmlspecialchars($_FILES[key($_FILES)]['name'] ?? 'файл');
                    logAction('UPDATE', 'projects', "Загружен новый скан к договору ID {$pid} (Файл: {$fileName})");
                }
                break;

            case 'upload_contract.php':
                $pid = (int)($_POST['project_id'] ?? 0);
                $fileName = htmlspecialchars($_FILES['contract_pdf']['name'] ?? 'документ');
